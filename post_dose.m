...
%animals = ["ee21" "ee22" "ee23" "ee24" "ee25" "ee26"];
animals = ["ee22_first_dose"];
...
for i=1:length(animals)
    ...
    % Load the CSV file
    activity_file = '/home/rbain/git/EE/Double dose - Band Group/' + animal + '/activity.csv';
    ...
    dose_file = '/home/rbain/git/EE/Double dose - Band Group/' + animal + '/doses.csv';
    ...
    % weird edge case where loading a single row csv ends up being split
    % into a table with 2 columns. One for date, one for time.
    if width(dose_data) == 2
        dose_data = table(datetime(strcat(datestr(dose_data.(1)), {' '}, datestr(dose_data.(2)))));
    end
    ...
    act_counts = act_counts / (dose_idx - 1);
